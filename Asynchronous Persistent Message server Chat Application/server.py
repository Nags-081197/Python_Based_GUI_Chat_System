...
def handle_client(client, msg):                                                                                 # function takes client object and msg frame object for displaying messages
    ...
    while True:                                                                                                 # infinite traversal of the following statements
        ...
        if user_name in user_names:                                                                             # checking whether the username is already used by any other connected clients on the server
        ...
        elif user_name !={quit} and not user_name.isspace() and user_name !="active_clients"and user_name !="all_clients":                   # checking whether the username is not {quit}, " " -just a space or "active_clients" as these are reserved for other funtionalities in the system
            ...
            all_clients[client] = user_name                                                                     # associating the client address, port no. etc with the user provided username
            if ((os.path.isfile(user_name+'.txt'))):                                                            # checking for the queue of messages (file created to store the same)
                pass                                                                                            # if the file is present do nothing and just pass
            else:                                                                                               # if the file is not present
                filetoread = str(user_name+ ".txt")                                                             # storing the file name to be the username
                fh = open(filetoread, 'w+')                                                                     # opening the file
...
def messaging(client, user_name):                                                                                                      # the function takes the forked client and its username as parameters
    while True:
        ...
        if temp_msg[:10]=="!@#$%^&*()":                                                                                                # checking for the first 10 elements to be a specific code
            if temp_msg[10]=="1":                                                                                                      # checking if the 10th index of the string temp_msg is equal to "1" meaning it is a unicast message
                broke_msg=(temp_msg.split("!@#$%^&*()", 5))                                                                            # splitting the complete temp_msg into 5 at every instance/occurrence of the secret code
                username_tosend=broke_msg[4]                                                                                           # assigning the usernametosend with split message list's 4th index which is the recipient username

                if username_tosend=='':                                                                                                 # checking if the usernametosend from the split msg is not empty or just space
                    msgtouser="SELECT A USER FROM THE ACTIVE CLIENTS LIST"                                                              # assigning a notifier string to msgtouser
                    clientaddresstosend = (list(clients.keys())[list(clients.values()).index(user_name)])                               # storing the key value(address value) associated with the username - actual senders address
                    send_to_unicast(msgtouser, clientaddresstosend, user_name)                                                          # calling the send_to_unicast function and passing msgtouser, the recipient client address & the sender's username

                else:
                    msgtouser=broke_msg[3]                                                                                                                      # taking up the msg from the split message list using the index 3
                    filetowrite = str(username_tosend+".txt")                                                                                                   # initializing the string variable with username.txt
                    fh = open(filetowrite,'a')                                                                                                                  # opening the file to write data into the file
                    dateTimeObj = str(datetime.now())                                                                                                           # getting the current date and time value
                    texttobewritten = "!@#$%^&*()"+msgtouser+" -- "+dateTimeObj+"!@#$%^&*()"+str(username_tosend)+"!@#$%^&*()"+user_name+"^%$#@!%^&*()"         # packing the message,username,date and time value,username to send
                    fh.write(texttobewritten)                                                                                                                   # writing the packed message onto the file
                    fh.close()                                                                                                                                  # closing the file after writing into it




            elif temp_msg[10]=="2":                                                                                                     # checking if the 10th index of the string temp_msg is equal to "2" meaning it is a multicast message
                broke_msg = (temp_msg.split("!@#$%^&*()", 5))                                                                           # splitting the complete temp_msg into 5 at every instance/occurrence of the secret code
                msgtouser = broke_msg[3]
                usernames_tosend_list = json.loads(broke_msg[4])                                                                        # converting the string into a
                if bool(usernames_tosend_list):                                                                                         # checking whether the list is empty or not
                    for x in (usernames_tosend_list):                                                                                   # traverse for every element in the list
                        filetowrite = str(x+".txt")                                                                                                         # initializing the file name with the username
                        fh = open(filetowrite, 'a')                                                                                                         # opening the file  to write the data into the file
                        dateTimeObj = str(datetime.now())                                                                                                   # getting the current date and time object
                        texttobewritten = "!@#$%^&*()" + msgtouser +" -- "+dateTimeObj +"!@#$%^&*()"+x+"!@#$%^&*()" + user_name + "^%$#@!%^&*()"            # packing the message with username,username to send, message to user
                        fh.write(texttobewritten)                                                                                                           # writing into th file the packed message
                        fh.close()                                                                                                                          # closing the file after writing into the file


                else:                                                                                                                   # checking if the list of usernames list is empty
                    msgtouser = "SELECT A USER FROM THE ACTIVE CLIENTS LIST"                                                            # notifier string with a msg to user
                    clientaddresstosend = (list(clients.keys())[list(clients.values()).index(user_name)])                               # storing the key value(address value) associated with the username - actual senders address
                    send_to_unicast(msgtouser, clientaddresstosend, user_name)                                                          # calling the send_to_unicast function and passing msgtouser, the recipient client address & the sender's username



            elif temp_msg[10]=="3":                                                                                                     # checking if the 10th index of the string temp_msg is equal to "2" meaning it is a multicast message
                broke_msg = (temp_msg.split("!@#$%^&*()", 5))                                                                           # splitting the complete temp_msg into 5 at every instance/occurrence of the secret code
                msgtouser = broke_msg[3]                                                                                                # taking up the actual msg for broadcast by the sender

                for x in (concurrent_usernames):                                                                                                  # traverse for every element in the list

                    filetowrite = str(x + ".txt")                                                                                                       # initializing the string variable with username.txt
                    fh = open(filetowrite, 'a')                                                                                                         # opening the file with append mode
                    dateTimeObj = str(datetime.now())                                                                                                   # getting current date and time
                    texttobewritten = "!@#$%^&*()" + msgtouser +" -- "+dateTimeObj+"!@#$%^&*()" + x + "!@#$%^&*()" + user_name + "^%$#@!%^&*()"         # packing a string with the message to the user,date and time,username and username to send
                    fh.write(texttobewritten)                                                                                                           # writing the contents onto the file
                    fh.close()                                                                                                                          # closing the file after writing into it


        elif message == bytes("message_request","utf8"):                                                                                 # checking if its a message_request
            if os.stat(user_name+'.txt').st_size > 0:                                                                                    # checking is the file contains any data or not
                filetoread = str(user_name+ ".txt")                                                                                      # assigning the filenae to read the data from
                fh = open(filetoread, 'r')                                                                                               # opening the file to read the message in it
                filestring = fh.read()                                                                                                   # reading the contents in file onto a string variable
                fh.close()                                                                                                               # closing the file after reading from it
                splitlist = filestring.split("^%$#@!%^&*()",100)                                                                         # splitting the string read from the file at specific intervals
                for x in range(len(splitlist)-1):                                                                                        # looping for the total number of split items
                    singlemsgstr = splitlist[x]                                                                                          # taking one piece of the split items into the singlemessage variable
                    broke_msg = (singlemsgstr.split("!@#$%^&*()", 5))                                                                    # splitting each single messages again to unpack or break the message
                    msgtouser = broke_msg[1]                                                                                             # getting the first index that is the message to the user
                    clientaddresstosend = (list(clients.keys())[list(clients.values()).index(broke_msg[2])])                             # getting the address of the username specified in the packed message
                    username_to_send = broke_msg[3]                                                                                      # getting the username to send from the third index value
                    send_to_unicast(msgtouser, clientaddresstosend, username_to_send)                                                    # sending the message to the specific user
                if os.path.exists(user_name+".txt"):                                                                                     # checking if the file exists
                    open(user_name+'.txt', 'w').close()                                                                                  # emptying the file after all the messages are delivered to the specific user

            else:
                msgtouser = "No Messages available !!"                                                                                      # assigning a notifier string to msgtouser
                clientaddresstosend = (list(clients.keys())[list(clients.values()).index(user_name)])                                       # storing the key value(address value) associated with the username - actual senders address
                send_to_unicast(msgtouser, clientaddresstosend, user_name)                                                                  # calling the function send_to_unicast with message- No messages available and corresponding username


        elif (message != bytes("{quit}", "utf8")) and (user_name in user_names) and (message!=bytes("active_clients","utf8")) and (message!=bytes("message_request","utf8"))and (message!=bytes("all_clients","utf8")): # checking whether its not a quit msg and active_clients and the sender is in the active clients list and all clients
        ...
        elif message == bytes("all_clients","utf8"):                                                                                 # checking if it a all_clients message
            listtosend = list(concurrent_usernames)                                                                                  # coverting the concurrent_usernames set into a list to pass it onto the client
            all_usernames_list = json.dumps(listtosend)                                                                              # coverting the list into a string using the json.dumps
            client.send(bytes(all_usernames_list, "utf8"))                                                                           # sending the string containing the list of active users is then sent to requested client
...
def send_to_all(msg,username):                                                                                  # the function accepts a message to be sent and the username of the user who is trying to send the message
    ...
    finalmsg = str(username)+" : "+str(msg)                                                                     # creating a final message with username and message passed to the function
...
def send_to_unicast(msg,client_add,user_name):                                                                  # the fucntion takes the message, recepient client address & senders username as parameter
        client_add.send(bytes(("\n"+user_name+" : "+msg+"\n"),"utf8"))                                          # send the username with the message to the intended recepient


#############################################################################################################################################################################################################################################
"""function deals with the sending of messages to more than one user (one-to-many) MULTICAST """
#############################################################################################################################################################################################################################################


def send_to_multicast(msg,clientaddresseslist,user_name):                                                       # the fucntion takes the message, recepient client address list & senders username as parameter
    for client in clientaddresseslist:                                                                          # traverses the loop for each recepient user in the list
        client.send(bytes((user_name+" : "+msg), "utf8"))                                                       # send the username with the message to the intended recepient


##############################################################################################################################################################
"""Function deals with the broadcast request from a certain user (one-to-all) BROADCAST"""
##############################################################################################################################################################


def send_to_broadcast(msg,username):                                                                    # the function accepts a message to be sent and the username of the user who is trying to send the message
    clientaddressestosend_list = []                                                                     # list used to store the username to client address mapping for each username
    for x in (user_names):                                                                              # traversing through all the active usernames presently connected to the server
        item_to_append = (list(clients.keys())[list(clients.values()).index(x)])                        # storing the key value(address value) associated with each username
        clientaddressestosend_list.append(item_to_append)                                               # adding each address values to the list
    ...
    for client in clientaddressestosend_list:                                                           # traversing each client address in the list
        client.send(bytes((username+" : "+msg),"utf8"))                                                 # send the message to all the users present in the list with the senders username
...
all_clients = {}                        # creating an empty dictionary to store the corresponding client username with the client address for all the clients/user logs
