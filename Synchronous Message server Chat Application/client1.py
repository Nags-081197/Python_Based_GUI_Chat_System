...
def receive():
    while True:
        ...
        if msg[0] != "[":                                                                                       # Checks if the msg starts with "[" since it reserved for different actions
            msg_list.insert(tkinter.END, msg)                                                                   # Inserting the msg onto the display region on the client interface
...
def message_to_send(event=None):                                                                                                     # event is passed by binders i.e. by a enter message send button
    ...
    if status == "YES":                                                                                                              # checking if the status is "YES" or not -  meaning this is connected/registered user or not
        ...
        message_to_send_frame.title(" Client 1 Message Tab ")                                                                        # Title for the child window
...
##############################################################################################################################################################################################################################################################################################################################
"""Funcation deals with getting a list of all active users from the server and giving a user selection option depending on unicast or multicast """
###############################################################################################################################################################################################################################################################################################################################
        ...
        if jsonstring[0] == "[":                                                                                        # checking whether it a string form of the list of the active clients
            ...
            if bool(activeclients_list):                                                                                # checking whther the list is empty or not
                ...
                if selected == "1":                                                                                     # checking if the selected variable is "1" meaning the selection of the unicast option
                    ...
                    for x in range(len(activeclients_list)):                                                            # traverse for the number of users present in the active clients  list
                        tkinter.Radiobutton(unicast,                                                                    # creation of radio buttons for each username for the selection of username - after selection username value itself is set ontot the variable
                                            text=activeclients_list[x],
                                            padx=20,
                                            variable=u,
                                            value=activeclients_list[x]).pack()
                    ...
                    for x in range(len(activeclients_list)):                                                            # traversing for the number os users present in the active_clients list
                        intvar_dict[activeclients_list[x]] = tkinter.IntVar()                                           # setting each username from the active clients list as a key and making them tkinter intvariable
                        c = tkinter.Checkbutton(multicast, text=activeclients_list[x],                                  # checkbuttons are created for the total number os active users from the list and which when selected the value is set or else it is not
                                                variable=intvar_dict[activeclients_list[x]])
...
top.title("Client_1")                                                                                                   # naming the client window/frame
