    ...
    while True:
        ...
        if temp_msg[:10]=="!@#$%^&*()":                                                                                                # checking for the first 10 elements to be a specific code
            if temp_msg[10]=="1":                                                                                                      # checking if the 10th index of the string temp_msg is equal to "1" meaning it is a unicast message
                ...
                broke_msg = (temp_msg.split("!@#$%^&*()", 5))                                                                           # splitting the complete temp_msg into 5 at every instance/occurrence of the secret code
                clientaddressestosend_list = []                                                                                         # creating a list of clientaddresses to send the message to
                ...
                if bool(usernames_tosend_list):                                                                                         # checking whether the list is empty or not
                    for x in (usernames_tosend_list):                                                                                   # traverse for every element in the list
                        item_to_append = (list(clients.keys())[list(clients.values()).index(x)])                                        # map/get the client addresses depending on the selected usernames list
                        clientaddressestosend_list.append(item_to_append)                                                               # add each address to the client address to send list
                ...
                msgtouser = broke_msg[3]                                                                                                # actual msg for the multicast recipient
                send_to_multicast(msgtouser, clientaddressestosend_list, user_name)                                                     # calling the send_to_multicast function and passing msgtouser, the recipient client address list & the sender's username
                ...
                send_to_broadcast(msgtouser,user_name)                                                                                  # calling the send_to_broadcast function and passing msgtouser & the sender's username


        elif (message != bytes("{quit}", "utf8")) and (user_name in user_names) and (message!=bytes("active_clients","utf8")):          # checking whther its not a quit msg and active_clients and the sender is in the active clients list
...
def send_to_all(msg,username):                                                                                  # the function accepts a message to be sent and the username of the user who is trying to send the message
    ...
    finalmsg = username+" : "+msg                                                                               # creating a final message with username and message passed to the function
